...
- There are false positives, and many detected conflicts are safe to ignore. Why? This addon considers any two keymap items that share the same key mapping in the same context to be a conflict. However, blender will stack shortcuts and use the operators' poll functions for detecting which is the correct operator to use in the current context. For example, vert select vs edge select is a stacked key in default blender mappings. Because it's nigh impossible to smartly detect these valid overlaps by digging through every operator's polling code, it just flags any stacked items as conflicts. 
